export_wp_ov <- function(model,pbp_data) {
    ...
    # Inputs:
    # model: a win probability model with the following 9 input features:
    #   first_drive, game_seconds_remaining, score_differential, down, ydstogo,
    #   yardline_100, home, posteam_timeouts_remaining, defteam_timeouts_remaining
    ...
    #   game_seconds_remaining: int, number of seconds remaining in game (0-900)
    #   quarter_seconds_remaining: int, number of seconds remaining in overtime (0-900)
    ...
    pbp_filtered <- pbp_data %>%
        mutate(down=(if_else(kickoff_attempt==1,1,down)),
               ...
               game_seconds_remaining =(if_else(kickoff_attempt==1,
                                                game_seconds_remaining+0.005,
                                                game_seconds_remaining)),
               elapsed_time = if_else(season>2016,
                                      4500-game_seconds_remaining,
                                      4200-game_seconds_remaining)) %>%
