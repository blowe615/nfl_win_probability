plot_wp_model_LOSO <- function(cv_results) {
    # A function to bin and plot predicted vs actual win probabilities for the
    # purpose of cross validation of a win probability model
    ...
    #   qtr: int, quarter label used for plotting
    #   wp: num, model assigned win probability
    #   label: int, 1 if possessing team won, otherwise 0
    
    # Output:
    # Plot of observed vs predicted win probability binned by quarter and 5%
    # win probability increments
    ...
    cv_results %>%
        ...
        # as a percentage of number of total plays in each quarter
        summarize(n_plays = n(),
                  n_wins = sum(label),
        ...
        labs(x="Predicted Win Probability",
             ...
             title="WP Model for 1999-2020 Seasons",
